...
soap_peek_element(struct soap *soap)
{
  ...
  *s = '\0';
  ...
  // @note: 记录接口TAG名称 by lxt
  if ( soap->part == SOAP_IN_BODY && soap->level == 2 )
  {
      if ( *soap->matchtag )
      {
		  strncpy(soap->matchtag2, soap->tag, sizeof(soap->matchtag2));
      }
	  else
	  {
		  strncpy(soap->matchtag, soap->tag, sizeof(soap->matchtag));
	  }
  }
...
{ if (s)
  {
    ...
    *p = strtoull(s, &r, 10);
	...
	// 20141107 xiaoyukai 用下代码替换函数sscanf，sscanf函数无法处理越界的字符串和负数字符串。
    *p = 0;
	const char * sz = s;
	*p = 0;
	while (*sz > 0 && *sz <= 32)
