...
// 2014.11.07 xiaoyukai 如果定义这个，使用windows平台的字符转无符号数字的函数，这个函数有bug，会将带符号的字符转为一个正数，这个不符合要求
// 注释后，使用soap自带的函数。
//#  define HAVE_STRTOL
//#  define HAVE_STRTOUL
...
{ short state;			/* 0 = uninitialized, 1 = initialized, 2 = copy of another soap struct */
  ...
  // @note 添加内部监控参数
  size_t sendcount;              // 发送包个数
  size_t recvcount;              // 接收包个数
  char   matchtag[SOAP_TAGLEN];  // 所属TAG
  char   matchtag2[SOAP_TAGLEN]; // 所属TAG2
  char   token[SOAP_TAGLEN];     // 所属添加TOKEN值
  bool   bRequestSucess;         // 是否是成功请求还是错误请示，
};
